...
기존에 학습된 **YOLOv8-Seg (차량 파손/부품 탐지)** 모델의 예측 결과를 활용하여, 추가적인 학습 없이 **차량의 촬영 방향(8면)**을 자동으로 분류하는 알고리즘입니다.
...
## ⚙️ 2. 알고리즘 로직: 점수 산정 (Scoring System)

YOLO가 탐지한 객체(`Confidence > 0.25`)를 분석하여 4방향 점수(`F`, `B`, `L`, `R`)를 누적합니다.
...
점수가 산정된 후, 아래의 **우선순위(Priority)** 에 따라 최종 뷰를 결정합니다.

### 🛑 Priority 1. 양안 시각 (Binocular Vision)
사람처럼 **"양쪽 눈(라이트)이 다 보이면 정면"** 이라고 판단합니다. 점수 계산보다 우선합니다.
- **Condition**: `Head lights(L)` AND `Head lights(R)` 감지됨
- **Result**: **`Front`** (측면 점수가 아무리 높아도 무시)

### ⚖️ Priority 2. 동점 방지 및 정면/후면 확정 (Tie-Breaking)
...
실제 데이터가 들어왔을 때 알고리즘이 어떻게 판단하는지 예시입니다.
...
# (classifier_utils.py에 해당 로직이 포함되어 있다고 가정)
from classifier_utils import classify_view_logic
...
# 2. 뷰 분류 (로직 실행)
# view: "Front-Left", "Back", etc.
# scores: {'F': 5, 'B': 0, 'L': 4, 'R': 0}
view, scores = classify_view_logic(results)

print(f"Predicted View: {view}")
