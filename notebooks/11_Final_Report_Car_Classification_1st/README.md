...
본 프로젝트의 연구 결과를 바탕으로, 실제 서비스(보험 처리, 렌터카 반납 등)에 즉시 도입할 수 있는 **2-Step 캐스케이드(Cascade) 추론 파이프라인**을 시범 구축했습니다.

가볍고 빠른 분류 모델과 정밀한 탐지 모델을 직렬로 연결하여, 서버의 연산 비용(GPU)을 최소화하면서도 정확도를 극대화하는 실무형 아키텍처입니다.



### 💡 파이프라인 작동 구조 (Process)

1. **데이터 추출:** Test 데이터셋에서 원하는 이미지(인덱스 번호 파라미터)를 불러옵니다.
2. **Step 1. 고속 필터링 (ResNet50):** * 추론 속도(149 FPS)가 가장 빠른 `ResNet50` 분류 모델을 1차 관문으로 배치합니다.
   * 이미지가 '차량 아님(non_car)'으로 판별되면, 불필요한 연산을 방지하기 위해 프로세스를 즉시 종료(**STOP**)합니다.
3. **Step 2. 정밀 BBox 추출 (YOLOv8):**
   * 1차 관문에서 '차량'으로 판별된 이미지에 한해서만 `YOLOv8` 객체 탐지 모델을 가동합니다.
   * 화면 내 승용차, 버스, 트럭 등의 메인 차량 윤곽을 정밀하게 탐지하여 Bounding Box를 그립니다.
4. **비율 산출 및 검증:**
   * 이미지 전체 면적 대비 차량 BBox의 면적 비율(%)을 계산하여 화면에 출력합니다.
